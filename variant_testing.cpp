...
TEST(variant, converting_ctor) {
  ...
  EXPECT_EQ(get<test_object>(b), 42);
...
TEST(variant, trivial_copy_ctor) {
  ...
  EXPECT_EQ(get<0>(b), 123);
  EXPECT_EQ(get<test_object>(b).assign(42), 42);
  EXPECT_EQ(get<test_object>(b_copy), 123);
  EXPECT_EQ(get<0>(b_copy), 123);
  EXPECT_EQ(get<test_object>(b), 42);
  EXPECT_THROW(get<double>(b), bad_variant_access);
  EXPECT_THROW(get<1>(b), bad_variant_access);
  ...
  EXPECT_THROW(get<test_object>(d), bad_variant_access);
  EXPECT_THROW(get<int64_t>(d), bad_variant_access);
  EXPECT_THROW(get<test_object>(d_copy), bad_variant_access);
...
struct non_trivial_copy {
  constexpr non_trivial_copy() : t(0) {}
  constexpr non_trivial_copy(int a) : t(a) {}
  constexpr non_trivial_copy(const non_trivial_copy& other) : t(other.t) {}
  constexpr non_trivial_copy(non_trivial_copy&& other) : t(other.t) {}

  constexpr int assign(int a) {
    t = a;
    return t;
  }

  friend constexpr bool operator==(non_trivial_copy const& a, non_trivial_copy const& b) {
    return a.t == b.t;
  }
...
TEST(variant, non_trivial_copy_ctor) {
  ...
  EXPECT_EQ(get<non_trivial_copy>(b_copy), 123);
  EXPECT_EQ(get<0>(b_copy), 123);
  EXPECT_EQ(get<non_trivial_copy>(b), 42);
  EXPECT_THROW(get<double>(b), bad_variant_access);
  EXPECT_THROW(get<1>(b), bad_variant_access);
  EXPECT_THROW(get<double>(b_copy), bad_variant_access);
  EXPECT_THROW(get<1>(b_copy), bad_variant_access);
  ...
  EXPECT_EQ(get<non_constexpr>(d_copy), 322);
  EXPECT_EQ(d_copy.index(), 1);
  EXPECT_THROW(get<non_trivial_copy>(d), bad_variant_access);
  EXPECT_THROW(get<int64_t>(d), bad_variant_access);
  EXPECT_THROW(get<non_trivial_copy>(d_copy), bad_variant_access);
  EXPECT_THROW(get<int64_t>(d_copy), bad_variant_access);
...
struct trivial_move {
  constexpr trivial_move() : t(0) {}
  constexpr trivial_move(int a) : t(a) {}
  constexpr trivial_move(trivial_move const& other) : t(other.t) {}
  constexpr trivial_move(trivial_move&& other) = default;
  ...
  friend constexpr bool operator==(trivial_move const& a, trivial_move const& b) {
    return a.t == b.t;
  }
...
TEST(variant, trivial_move_ctor) {
  ...
  EXPECT_EQ(get<trivial_move>(b_move), 42);
...
template
struct variant<std::string, test_object, non_trivial_copy, non_constexpr>;
